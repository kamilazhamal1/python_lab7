...
class Form3(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.listWidget.clicked.connect(self.handleActivated)
        self.btn_back.clicked.connect(self.back)
        self.btn_next.clicked.connect(self.next)

    def handleActivated(self):
        answers[1] = str(self.listWidget.currentItem().text())
        self.label_selected.setText('Выбрано: ' + answers[1])
...
class Form4(QtWidgets.QMainWindow):
    ...
    def handleActivated(self, row, column):
        answers[2] = self.tableWidget.item(row, column).text()
        self.label_selected.setText('Выбрано: ' + answers[2])
